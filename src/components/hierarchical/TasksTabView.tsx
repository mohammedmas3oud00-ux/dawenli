...
export const TasksTabView: React.FC<TasksTabViewProps> = ({
  ...
  return (
    <div className="space-y-4 max-w-5xl mx-auto">
      ...
      <div className="bg-white dark:bg-[#131d18] p-4 sm:p-5 rounded-2xl border border-[#e8e4db] dark:border-[#26372d] shadow-2xs space-y-3.5 transition-colors">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-base font-bold text-[#1a2420] dark:text-white">قائمة المهام (Tasks)</h1>
        ...
        {/* Filter Toolbar */}
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-[#f0ede6] dark:border-[#223028]">
          <div className="flex items-center bg-[#f4f2ec] dark:bg-[#192620] p-0.5 rounded-xl text-xs">
            <button
              type="button"
          ...
          <CustomSelect
            ...
            prefixIcon={<Filter className="w-3 h-3 text-[#7d8982] dark:text-[#8ea095]" />}
            size="xs"
            buttonClassName="rounded-lg py-1 px-2.5 bg-[#f8f7f4] dark:bg-[#192620] border-[#d8d4cc] dark:border-[#283830]"
          />
        </div>
      </div>
            ...
            return (
              <div
                ...
                className={`p-3.5 sm:p-4 flex items-start justify-between gap-3 transition-colors group ${
                  isDone ? 'bg-[#faf9f6] dark:bg-[#16211a]' : 'hover:bg-[#fcfbfa] dark:hover:bg-[#18261e]'
                }`}
              >
                  ...
                  <div className="space-y-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                      ...
                      {task.priority === 'medium' && (
                        <span className="text-[11px] text-amber-700 dark:text-amber-400 font-medium flex items-center gap-0.5">
                    ...
                    {task.description && (
                      <p className={`text-[11px] leading-relaxed line-clamp-2 ${isDone ? 'text-[#99a39c] dark:text-[#6a7c71]' : 'text-[#636e67] dark:text-[#9bb0a3]'}`}>
                        {task.description}
                      </p>
                    )}

                    <div className="flex flex-wrap items-center gap-3 pt-0.5 text-[11px] text-[#7d8982] dark:text-[#8ea095]">
                      {parentProject && (
                        <button
                          type="button"
                ...
                <div className="flex items-center gap-1 shrink-0 opacity-80 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                  {onStartFocus && !isDone && (
                    <button
                      ...
                      className="p-1.5 text-[#174235] dark:text-emerald-400 hover:bg-[#ebf4f0] dark:hover:bg-[#1a2b22] rounded-lg transition-colors cursor-pointer"
                      title="بدء جلسة تركيز على هذه المهمة"
                      aria-label="بدء جلسة تركيز على هذه المهمة"
                    >
